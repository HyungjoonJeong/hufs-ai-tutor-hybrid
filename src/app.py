...
[모바일 최적화 수식/기호 규칙 - 필수]
1. LaTeX 수식 안에서 \\text{...} 구문을 절대 사용하지 마라. (에러의 원인임)
2. 통화 기호(￦)나 단위(원, 개)는 수식($) 기호 '밖'에 일반 텍스트로 적어라.
   - 나쁜 예: $ \\text{￦} 10,000 $
   - 좋은 예: ￦ $ 10,000 $
3. 분수나 복잡한 수식은 가급적 $$ 를 사용하여 별도의 줄에 표기하라.
4. 숫자 사이의 콤마(,)는 수식 기호 없이 일반 텍스트로 작성하라.
